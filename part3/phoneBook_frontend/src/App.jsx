...
const PersonForm = ({
  ...
  const addPerson = (event) => {
    ...
    if (nameExists && numberExists) {
      ...
      if (
        window.confirm(
          `${personObject.name} name is already added to phonebook, replace the old number with a new one?`
        )
      ) {
...
const App = () => {
  ...
  return (
    <div>
      ...
      <Filter showAll={showAll} setShowAll={setShowAll} newFilter={newFilter} setNewFilter={setNewFilter} />
      <h3>Add a new</h3>
      ...
      />
      <h3>Numbers </h3>
      <Persons namesToShow={namesToShow} handleDelete={handleDelete} setMessage={setMessage} />
    </div>
  );
};
